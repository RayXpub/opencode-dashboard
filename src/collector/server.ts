...
type CollectorConfigBase = {
  hostname: string;
  port: number;
};

type CollectorConfig = CollectorConfigBase &
  (
    | { dashboard: Bun.HTMLBundle; dashboardRoot?: never }
    | { dashboard?: never; dashboardRoot: string }
  );

export function createServer(config: CollectorConfig) {
  const staticRoot = config.dashboardRoot;
  const dashboardRoute = staticRoot
    ? {
        GET() {
          return new Response(
            Bun.file(path.join(staticRoot, "web/index.html")),
          );
        },
      }
    : config.dashboard!;
  ...
  const server = Bun.serve({
    ...
    routes: {
      ...
      "/api/v1/heartbeat": {
        async POST(req) {
          ...
          const commands = parsed.data.source.capabilities?.includes(
            "session.write",
          )
            ? claimSessionCommands()
            : [];
    ...
    async fetch(req) {
      ...
      if (!filePath.startsWith(`${path.resolve(staticRoot)}${path.sep}`)) {
    ...
    development: staticRoot ? false : { hmr: true, console: true },
  });
